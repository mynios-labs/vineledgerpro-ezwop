...
-   **eBay-Compliant Listing Creation:** Overhauled draft listing creation with AI-generated structured descriptions, 80-character title truncation with real-time indicators, a category search interface, and pre-flight validation. The publish route (`POST /api/listings/publish`) uses the centralized EbayClient class for all eBay API interactions (upsertInventoryItem, createOffer, publishOffer) ensuring proper header configuration.
-   **Draft Page Edit Mode:** Fully operational edit workflow with GET /api/listings/:id endpoint providing complete listing data. Edit mode pre-populates all fields (title, price, dimensions, weight, category, fulfillment policy, SKU, photos) with relaxed validation (photos optional, privacy checks bypassed). Form sections (Category, Fulfillment Policy) display correctly in both create and edit modes via guard pattern `{(vineItem || listing) &&`. Race condition fixed by preventing reset effect from clearing pre-populated values when listingId exists.
